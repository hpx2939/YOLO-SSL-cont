...
class ASFF_Detect(nn.Module):   #add ASFFV5 layer and Rfb 
    ...
    def __init__(self, nc=80, anchors=(), ch=(), multiplier=0.5,rfb=False, inplace=True):  # detection layer
        ...
        self.na = len(anchors[0]) // 2  # number of anchors
        self.grid = [torch.zeros(1)] * self.nl  # init grid
        self.l0_fusion = ASFFV54h(level=0, multiplier=multiplier,rfb=rfb)
        self.l1_fusion = ASFFV54h(level=1, multiplier=multiplier,rfb=rfb)
        self.l2_fusion = ASFFV54h(level=2, multiplier=multiplier,rfb=rfb)
        self.l3_fusion = ASFFV54h(level=3, multiplier=multiplier,rfb=rfb)
...
class ASFF_Detect3h(nn.Module):   #add ASFFV5 layer and Rfb 
    stride = None  # strides computed during build
    export = False  # onnx export
    end2end = False
    include_nms = False
...
class Model(nn.Module):
    ...
    def forward_once(self, x, profile=False):
        ...
        for m in self.model:
            ...
            if profile:
                c = isinstance(m, (Detect, IDetect, IAuxDetect, IBin, Decoupled_Detect))
            ...
            # feature_vis = True
            # # # if m.i=='129, 132, 135, 138' and feature_vis: #選層數
            # if m.type == 'models.common.SPD' and feature_vis: #選nn block
            # # if m.type == 'Upsample' and feature_vis:
            #     print(m.type, m.i)
    ...
    def fuse(self):  # fuse model Conv2d() + BatchNorm2d() layers
        ...
        for m in self.model.modules():
            ...
            elif isinstance(m, (IDetect, IAuxDetect, Decoupled_Detect)):
                m.fuse()
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    # from(當前層輸入來自哪些層), number(當前層次數 初定), module(當前層類別), args(當前層類參數 初定)
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        # c1: 當前層的輸入的channel數  c2: 當前層的輸出的channel數(初定)  ch: 記錄著所有層的輸出channel
        if m in [nn.Conv2d, Conv,  RobustConv, RobustConv2, DWConv, GhostConv,  RepConv,  RepConv_OREPA, DownC, Partial_conv3, 
                 CrossConv, nn.ConvTranspose2d, GSConv, GSMConv, SCConv, Involution,
                 SPP, SPPF, SPPFCSPC, SimSPPF, SPPCSPC, ASPP, BASPP, GhostSPPCSPC, MixConv2d, Focus, Stem, GhostStem,  
                 Bottleneck, BottleneckCSPA, BottleneckCSPB, BottleneckCSPC, BasicRFB, CoT3,
                 RepBottleneck, RepBottleneckCSPA, RepBottleneckCSPB, RepBottleneckCSPC,  BasicConv, 
                 ...
                 SwinTransformer2Block, ST2CSPA, ST2CSPB, ST2CSPC, CBAM, MHSA,BoT3]:
            c1, c2 = ch[f], args[0]
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)

            args = [c1, c2, *args[1:]] #[ch_out, k, s] => [ch_in, ch_out, k, s]
            if m in [DownC, SPPCSPC, GhostSPPCSPC, CoT3, BoT3,
                ...
                args.insert(2, n)  # number of repeats [ch_in, ch_out, k, s] => [ch_in, ch_out, n, k, s]
                n = 1 #初始化
        elif m is nn.BatchNorm2d:
            args = [ch[f]]
        elif m is Concat:
            c2 = sum([ch[x] for x in f])
        elif m in [Concat_BiFPN2, Concat_BiFPN3]:
            ...
            c2 = ch[f] // 2
        elif m in [Detect, IDetect, Decoupled_Detect, ASFF_Detect,ASFF_Detect3h, IAuxDetect, IBin, IKeypoint]:
