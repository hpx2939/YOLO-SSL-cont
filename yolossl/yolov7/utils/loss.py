    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                #iou = bbox_alpha_iou(pbox.T, tbox[i], x1y1x2y2=False, SIoU=True)
                
                ##### nwd #####
                nwd_ratio = 0.5  # 平衡nwd和原始iou的權重
                nwd = wasserstein_loss(pbox, tbox[i]).squeeze()
                lbox += (1 - nwd_ratio) * (1.0 - nwd).mean() + nwd_ratio * (1.0 - iou).mean()  # iou loss
                ##### nwd #####
                
                # lbox += (1.0 - iou).mean()  # iou loss 原本的
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model   
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                #iou = bbox_alpha_iou(pbox.T, selected_tbox, x1y1x2y2=False, SIoU=True) 
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model   
        ...
        for i in range(self.nl):  # layer index, layer predictions
            ...
            if n_aux:
                ...
                iou_aux = bbox_iou(pbox_aux.T, selected_tbox_aux, x1y1x2y2=False, SIoU=True)  # iou(prediction, target)
                #iou_aux = bbox_alpha_iou(pbox_aux.T, selected_tbox_aux, x1y1x2y2=False, SIoU=True)
...
def compute_distillation_output_loss(p, t_p, model, d_weight=1):
    ...
    """
    t_ft = torch.cuda.FloatTensor if t_p[0].is_cuda else torch.Tensor  # trans data format
...
def compute_distillation_output_loss2(p, t_p, model, dist_loss="l2", T=20, reg_norm=None):
    """
    ...
    model : TYPE
        DESCRIPTION.
    dist_loss : TYPE, optional
        DESCRIPTION. The default is "l2".
    T : TYPE, optional
        DESCRIPTION. The default is 20.
    reg_norm : TYPE, optional
