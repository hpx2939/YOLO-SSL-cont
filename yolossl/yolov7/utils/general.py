...
def wasserstein_loss(pred, target, eps=1e-7, cst=12.8):
    ...
    c2 = target[:, :2]
    cmean = c1[:, :2] - c2[:, :2]
    center_dist = cmean[:, 0] * cmean[:, 0] + cmean[:, 1] * cmean[:, 1] + eps
    ...
    wh_dist = ((w1 - w2) ** 2 + (h1 - h2) ** 2) / 4 + eps
    wasserstein_dist = center_dist + wh_dist
    return torch.exp(-torch.sqrt(wasserstein_dist) / cst)
...
def bbox_alpha_iou(box1, box2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU=False, SIoU=False, alpha=2, eps=1e-9):
...
def iou_nms(boxes, scores, iou_thres, GIoU=False, DIoU=True, CIoU=False,SIoU=False):
    """
    ...
    while idx.numel() > 0:
        ...
        # 計算iou,選擇GIOU,DIOU,CIOU
        iou = bbox_iou(boxes[index, :], boxes[idx[1:], :], GIoU=GIoU, DIoU=DIoU, CIoU=CIoU, SIoU=SIoU)
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    prediction: [batch,num_anchors(yolo預設層),(xywh conf num_classes)]
    Returns:
         list of detections, on (n,6) tensor per image [xyxy, conf, cls]
    """

    nc = prediction.shape[2] - 5  # number of classes
    xc = prediction[..., 4] > conf_thres  # candidates 第4個是confidence

    # Settings
    min_wh, max_wh = 2, 4096  # (pixels) minimum and maximum box width and height預測物體的寬高大小限制
    max_det = 300  # maximum number of detections per image
    max_nms = 30000  # maximum number of boxes into torchvision.ops.nms()
    time_limit = 10.0  # seconds to quit after 超過這個時間就退出
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # If none remain process next image 如果前面過濾沒有bbox就下一張圖
        if not x.shape[0]:
            continue
        ...
        if nc == 1:
            ...
            x[:, 5:] *= x[:, 4:5]  # conf = obj_conf * cls_conf  (5以後都是obj)
        ...
        else:  # best class only  如果只有一個類別，選取最好的
        ...
        if not n:  # no boxes 沒有bbox就下一張圖
            continue
        elif n > max_nms:  # excess boxes 如果還有框就排序
        ...
        # i = diou_box_nms(boxes=boxes, scores=scores, iou_thres = iou_thres)
